...
const ImgCard = (props) => {
  ...
  const submitHandler = async (e) => {
    ...
    await firebaseDB.child(res).push(
      crypto,
      err => {
        if (err) {
          console.log(err);
  ...
  var parsedPrice = parseFloat(props.price);
  var newPrice = isNaN(parsedPrice) ? props.price : parsedPrice.toFixed(4);
  ...
  return (
    <>
      ...
      <p><button onClick={submitHandler} className='but2'>Add into your Wishlist : &nbsp;<svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" fill="gold" className="bi bi-heart-fill" viewBox="0 0 16 16">
